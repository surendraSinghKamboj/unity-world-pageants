...
const handler = async (req, res)=>{
    if (req.method === "GET") {
        const { id  } = req.query;
        try {
            const response = await _models_contestants__WEBPACK_IMPORTED_MODULE_1__/* ["default"].findById */ .Z.findById(id).select("images");
            if (response) {
                res.status(200).json({
                    response
                });
...
var __webpack_exports__ = __webpack_require__.X(0, [8550], () => (__webpack_exec__(9832)));
